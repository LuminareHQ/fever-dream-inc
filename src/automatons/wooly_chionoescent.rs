...
static DISTANCE_FROM_ORIGIN: f32 = 6.0;
...
fn update_based_on_owned(
    ...
    asset_server: Res<AssetServer>,
    mut wooly_chionoescents: Query<(Entity, &mut Automaton, &mut Transform, &WoolyChionoescent)>,
) {
    let quantity_owned =
    ...
    if quantity_owned != current_count {
        ...
        for (i, (_entity, _automaton, mut transform, _)) in wooly_chionoescents.iter_mut().enumerate() {
