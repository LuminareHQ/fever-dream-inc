...
const DISTANCE_FROM_ORIGIN: f32 =
    get_stats(AutomatonVariant::WoolyChionoescent).distance_from_origin;
const COOLDOWN: f32 = get_stats(AutomatonVariant::WoolyChionoescent).cooldown;
const CURRENCY_PER_TICK: u64 = get_stats(AutomatonVariant::WoolyChionoescent).currency_per_tick;
const SCALE: f32 = get_stats(AutomatonVariant::WoolyChionoescent).scale;
const ROTATION: f32 = get_stats(AutomatonVariant::WoolyChionoescent).rotation;

#[derive(Component)]
struct WoolyChionoescent;
...
fn setup(
    ...
    commands
        ...
        .observe(update_interface_state::<Pointer<Over>>(Some(
            crate::data::AutomatonVariant::WoolyChionoescent,
        )))
        .observe(click);
}

fn click(_: On<Pointer<Click>>, mut game_data: ResMut<GameData>) {
    game_data.purchase_source(crate::data::AutomatonVariant::WoolyChionoescent);
}
...
fn update_based_on_owned(
    ...
    asset_server: Res<AssetServer>,
    mut wooly_chionoescents: Query<(&mut Automaton, &mut Transform), With<WoolyChionoescent>>,
) {
    let quantity_owned =
        game_data.get_quantity_owned_by_source(crate::data::AutomatonVariant::WoolyChionoescent);
    ...
    if quantity_owned != current_count {
        ...
        commands.spawn((
            ...
            SceneRoot(scene),
            Automaton {
                source: crate::data::AutomatonVariant::WoolyChionoescent,
        ...
        for (i, (_automaton, mut transform)) in wooly_chionoescents.iter_mut().enumerate() {
...
fn movement(
    mut query: Query<(&mut Transform, &Automaton), With<WoolyChionoescent>>,
    time: Res<Time>,
) {
