...
const DISTANCE_FROM_ORIGIN: f32 = get_stats(AutomatonVariant::GapingDubine).distance_from_origin;
const COOLDOWN: f32 = get_stats(AutomatonVariant::GapingDubine).cooldown;
const CURRENCY_PER_TICK: u64 = get_stats(AutomatonVariant::GapingDubine).currency_per_tick;
const SCALE: f32 = get_stats(AutomatonVariant::GapingDubine).scale;
const ROTATION: f32 = get_stats(AutomatonVariant::GapingDubine).rotation;
...
impl Plugin for GapingDubinePlugin {
...
fn setup(
    ...
    // Preloading the scene so that it doesn't cause a hitch when we first spawn a GapingDubine
    let _: Handle<Scene> =
        asset_server.load(GltfAssetLabel::Scene(0).from_asset("models/gaping_dubine.glb"));
    ...
    commands
        ...
        .observe(update_interface_state::<Pointer<Over>>(Some(
            crate::data::AutomatonVariant::GapingDubine,
        )))
        .observe(click);
}

fn click(_: On<Pointer<Click>>, mut game_data: ResMut<GameData>) {
    game_data.purchase_source(crate::data::AutomatonVariant::GapingDubine);
}
...
fn update_based_on_owned(
    ...
    asset_server: Res<AssetServer>,
    mut gaping_dubines: Query<(Entity, &mut Automaton, &mut Transform, &GapingDubine)>,
) {
    let quantity_owned =
        game_data.get_quantity_owned_by_source(crate::data::AutomatonVariant::GapingDubine);
    let current_count = gaping_dubines.iter().count() as u64;
    if quantity_owned != current_count {
        // Load the GapingDubine scene
        ...
        // Spawn one new GapingDubine at its position on the circle
        ...
        commands.spawn((
            GapingDubine,
            Name::new("GapingDubine"),
            SceneRoot(scene),
            Automaton {
                source: crate::data::AutomatonVariant::GapingDubine,
        ...
        for (i, (_entity, _automaton, mut transform, _)) in gaping_dubines.iter_mut().enumerate() {
...
fn movement(mut query: Query<(&mut Transform, &Automaton), With<GapingDubine>>, time: Res<Time>) {
