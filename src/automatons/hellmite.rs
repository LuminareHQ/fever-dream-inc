...
const DISTANCE_FROM_ORIGIN: f32 = get_stats(AutomatonVariant::Hellmite).distance_from_origin;
const COOLDOWN: f32 = get_stats(AutomatonVariant::Hellmite).cooldown;
const CURRENCY_PER_TICK: u64 = get_stats(AutomatonVariant::Hellmite).currency_per_tick;
const SCALE: f32 = get_stats(AutomatonVariant::Hellmite).scale;
const ROTATION: f32 = get_stats(AutomatonVariant::Hellmite).rotation;
...
fn setup(
    ...
    // Preloading the scene so that it doesn't cause a hitch when we first spawn a hellmite
    let _: Handle<Scene> =
    ...
    commands
        ...
        .observe(update_interface_state::<Pointer<Over>>(Some(
            crate::data::AutomatonVariant::Hellmite,
        )))
        .observe(click);
}
...
fn update_based_on_owned(
    ...
    asset_server: Res<AssetServer>,
    mut hellmites: Query<(&mut Automaton, &mut Transform), With<Hellmite>>,
) {
    let quantity_owned =
        game_data.get_quantity_owned_by_source(crate::data::AutomatonVariant::Hellmite);
    let current_count = hellmites.iter().count() as u64;
    if quantity_owned != current_count {
        // Load the hellmite scene
        ...
        // Spawn one new hellmite at its position on the circle
        ...
        commands.spawn((
            ...
            SceneRoot(scene),
            Automaton {
                source: crate::data::AutomatonVariant::Hellmite,
        ...
        for (i, (_automaton, mut transform)) in hellmites.iter_mut().enumerate() {
...
fn movement(mut query: Query<(&mut Transform, &Automaton), With<Hellmite>>, time: Res<Time>) {
