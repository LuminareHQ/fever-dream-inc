...
fn update_based_on_owned(
    ...
    asset_server: Res<AssetServer>,
    mut hellmites: Query<(Entity, &mut Automaton, &mut Transform, &Hellmite)>,
) {
    let quantity_owned =
        game_data.get_quantity_owned_by_source(crate::data::IncomeSource::Hellmite);
    ...
    if quantity_owned != current_count {
        ...
        commands.spawn((
            ...
            SceneRoot(scene),
            Automaton {
                source: crate::data::IncomeSource::Hellmite,
        ...
        for (i, (_entity, _automaton, mut transform, _)) in hellmites.iter_mut().enumerate() {
