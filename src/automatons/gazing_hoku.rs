...
pub struct GazingHokuPlugin;

use crate::config::get_stats;
use crate::data::AutomatonVariant;
const DISTANCE_FROM_ORIGIN: f32 = get_stats(AutomatonVariant::GazingHoku).distance_from_origin;
const COOLDOWN: f32 = get_stats(AutomatonVariant::GazingHoku).cooldown;
const CURRENCY_PER_TICK: u64 = get_stats(AutomatonVariant::GazingHoku).currency_per_tick;
const SCALE: f32 = get_stats(AutomatonVariant::GazingHoku).scale;
const ROTATION: f32 = get_stats(AutomatonVariant::GazingHoku).rotation;
...
impl Plugin for GazingHokuPlugin {
...
fn setup(
    ...
    // Preloading the scene so that it doesn't cause a hitch when we first spawn a GazingHoku
    let _: Handle<Scene> =
        asset_server.load(GltfAssetLabel::Scene(0).from_asset("models/gazing_hoku.glb"));
    ...
    commands
        ...
        .observe(update_interface_state::<Pointer<Over>>(Some(
            crate::data::AutomatonVariant::GazingHoku,
        )))
        .observe(click);
}

fn click(_: On<Pointer<Click>>, mut game_data: ResMut<GameData>) {
    game_data.purchase_source(crate::data::AutomatonVariant::GazingHoku);
}
...
fn update_based_on_owned(
    ...
    asset_server: Res<AssetServer>,
    mut gazing_hokus: Query<(Entity, &mut Automaton, &mut Transform, &GazingHoku)>,
) {
    let quantity_owned =
        game_data.get_quantity_owned_by_source(crate::data::AutomatonVariant::GazingHoku);
    let current_count = gazing_hokus.iter().count() as u64;
    if quantity_owned != current_count {
        // Load the GazingHoku scene
        ...
        // Spawn one new GazingHoku at its position on the circle
        ...
        commands.spawn((
            ...
            SceneRoot(scene),
            Automaton {
                source: crate::data::AutomatonVariant::GazingHoku,
        ...
        for (i, (_entity, _automaton, mut transform, _)) in gazing_hokus.iter_mut().enumerate() {
...
fn movement(mut query: Query<(&mut Transform, &Automaton), With<GazingHoku>>, time: Res<Time>) {
