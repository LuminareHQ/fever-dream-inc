...
pub struct PelteLacertePlugin;

use crate::config::get_stats;
use crate::data::AutomatonVariant;
const DISTANCE_FROM_ORIGIN: f32 = get_stats(AutomatonVariant::PelteLacerte).distance_from_origin;
const COOLDOWN: f32 = get_stats(AutomatonVariant::PelteLacerte).cooldown;
const CURRENCY_PER_TICK: u64 = get_stats(AutomatonVariant::PelteLacerte).currency_per_tick;
const SCALE: f32 = get_stats(AutomatonVariant::PelteLacerte).scale;
const ROTATION: f32 = get_stats(AutomatonVariant::PelteLacerte).rotation;
...
impl Plugin for PelteLacertePlugin {
...
fn setup(
    ...
    // Preloading the scene so that it doesn't cause a hitch when we first spawn a PelteLacerte
    let _: Handle<Scene> =
        asset_server.load(GltfAssetLabel::Scene(0).from_asset("models/pelte_lacerte.glb"));
    ...
    commands
        ...
        .observe(update_interface_state::<Pointer<Over>>(Some(
            crate::data::AutomatonVariant::PelteLacerte,
        )))
        .observe(click);
}

fn click(_: On<Pointer<Click>>, mut game_data: ResMut<GameData>) {
    game_data.purchase_source(crate::data::AutomatonVariant::PelteLacerte);
}
...
fn update_based_on_owned(
    ...
    asset_server: Res<AssetServer>,
    mut pelte_lacertes: Query<(&mut Automaton, &mut Transform), With<PelteLacerte>>,
) {
    let quantity_owned =
        game_data.get_quantity_owned_by_source(crate::data::AutomatonVariant::PelteLacerte);
    let current_count = pelte_lacertes.iter().count() as u64;
    if quantity_owned != current_count {
        // Load the PelteLacerte scene
        ...
        // Spawn one new PelteLacerte at its position on the circle
        ...
        commands.spawn((
            PelteLacerte,
            Name::new("PelteLacerte"),
            SceneRoot(scene),
            Automaton {
                source: crate::data::AutomatonVariant::PelteLacerte,
        ...
        for (i, (_automaton, mut transform)) in pelte_lacertes.iter_mut().enumerate() {
...
fn movement(mut query: Query<(&mut Transform, &Automaton), With<PelteLacerte>>, time: Res<Time>) {
