...
static DISTANCE_FROM_ORIGIN: f32 = 5.0;
...
fn setup(
    ...
    commands
        ...
        .observe(update_interface_state::<Pointer<Over>>(Some(
            crate::data::IncomeSource::PelteLacerte,
        )))
        .observe(click);
}

fn click(_: On<Pointer<Click>>, mut game_data: ResMut<GameData>) {
    game_data.purchase_source(crate::data::IncomeSource::PelteLacerte);
}
...
fn update_based_on_owned(
    ...
    asset_server: Res<AssetServer>,
    mut pelte_lacertes: Query<(Entity, &mut Automaton, &mut Transform, &PelteLacerte)>,
) {
    let quantity_owned =
        game_data.get_quantity_owned_by_source(crate::data::IncomeSource::PelteLacerte);
    ...
    if quantity_owned != current_count {
        ...
        commands.spawn((
            ...
            SceneRoot(scene),
            Automaton {
                source: crate::data::IncomeSource::PelteLacerte,
        ...
        for (i, (_entity, _automaton, mut transform, _)) in pelte_lacertes.iter_mut().enumerate() {
