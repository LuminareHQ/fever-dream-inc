...
const DISTANCE_FROM_ORIGIN: f32 = get_stats(AutomatonVariant::Abyssopod).distance_from_origin;
const COOLDOWN: f32 = get_stats(AutomatonVariant::Abyssopod).cooldown;
const CURRENCY_PER_TICK: u64 = get_stats(AutomatonVariant::Abyssopod).currency_per_tick;
const SCALE: f32 = get_stats(AutomatonVariant::Abyssopod).scale;
const ROTATION: f32 = get_stats(AutomatonVariant::Abyssopod).rotation;
...
impl Plugin for AbyssopodPlugin {
...
fn setup(
    ...
    // Preloading the scene so that it doesn't cause a hitch when we first spawn a Abyssopod
    let _: Handle<Scene> =
        asset_server.load(GltfAssetLabel::Scene(0).from_asset("models/abyssopod.glb"));
    ...
    commands
        ...
        .observe(update_interface_state::<Pointer<Over>>(Some(
            crate::data::AutomatonVariant::Abyssopod,
        )))
        .observe(click);
}

fn click(_: On<Pointer<Click>>, mut game_data: ResMut<GameData>) {
    game_data.purchase_source(crate::data::AutomatonVariant::Abyssopod);
}
...
fn update_based_on_owned(
    ...
    asset_server: Res<AssetServer>,
    mut abyssopods: Query<(&mut Automaton, &mut Transform), With<Abyssopod>>,
) {
    let quantity_owned =
        game_data.get_quantity_owned_by_source(crate::data::AutomatonVariant::Abyssopod);
    let current_count = abyssopods.iter().count() as u64;
    if quantity_owned != current_count {
        // Load the Abyssopod scene
        ...
        // Spawn one new Abyssopod at its position on the circle
        ...
        commands.spawn((
            ...
            SceneRoot(scene),
            Automaton {
                source: crate::data::AutomatonVariant::Abyssopod,
        ...
        for (i, (_automaton, mut transform)) in abyssopods.iter_mut().enumerate() {
...
fn movement(mut query: Query<(&mut Transform, &Automaton), With<Abyssopod>>, time: Res<Time>) {
