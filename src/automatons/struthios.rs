...
impl Plugin for StruthiosPlugin {
...
fn setup(
    ...
    // Preloading the scene so that it doesn't cause a hitch when we first spawn a Struthios
    let _: Handle<Scene> =
        asset_server.load(GltfAssetLabel::Scene(0).from_asset("models/struthios.glb"));
...
fn update_based_on_owned(
    ...
    asset_server: Res<AssetServer>,
    mut struthioss: Query<(Entity, &mut Automaton, &mut Transform)>,
) {
    let quantity_owned =
    ...
    if quantity_owned != current_count {
        ...
        // Spawn one new Struthios at its position on the circle
        ...
        for (i, (_entity, _automaton, mut transform)) in struthioss.iter_mut().enumerate() {
